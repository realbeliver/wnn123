...
// -----------------------------------------------------------------------------
// tb_C2S0285_1n.v  (Verilog testbench for tt_um_wnn)
...
module tb;
    ...
    function automatic signed [15:0] real_to_q8p8;
        ...
        begin
            t = r * 256.0;
            if (t > 32767.0) 
                real_to_q8p8 = 16'h7FFF;
            else if (t < -32768.0) 
                real_to_q8p8 = 16'h8000;
            else 
                real_to_q8p8 = $rtoi(t);
        end
    endfunction
    ...
    // Golden WNN neuron: w * z * exp(-0.5*z^2),  z = (x-t)/d
    ...
    task automatic run_test;
        input real  x_real;
        input [7:0] grp; // Fits a single char string like "A"
        reg [15:0]  x_q88, hw_raw;
        real        hw_real, gld, diff, tol;
        reg [31:0]  verdict; // Fits standard string output "PASS"/"FAIL"
        begin
            x_q88 = real_to_q8p8(x_real);
            gld   = sum_golden(x_real);
            tol   = abs_real(gld) * 0.06 + 0.1; // 6% relative + 0.1 absolute floor
    ...
    initial begin
        ...
        // =====================================================================
        $display("\n================================================================");
        $display("  WNN Single-Neuron Top - Comprehensive Testbench");
        $display("  75 test vectors | 1 neuron | sim clock 10 ns");
        $display("================================================================");
        $display("\n--- PHASE 1: Configuring 1 neuron ---");

        load_neuron(0, 1.0, 0.0, 0.80);
        $display("  -> 1 neuron loaded.\n");
        ...
        // =====================================================================
        $display("--- GROUP H: Post-reconfiguration (uniform w=1,t=0,d=1) ---");
        run_test(  0.0,  "H");
        run_test(  1.0,  "H");
        run_test( -1.0,  "H");
        run_test(  2.0,  "H");
    ...
    // -------------------------------------------------------------------------
    // Concurrent assertions equivalent checks (6 properties)
    // -------------------------------------------------------------------------

    // A: ready (registered) and x_latch_valid must never overlap
    ...
    // C: ready must imply pipeline not busy
    always @(posedge clk) begin
        if (rst_n) begin
            if (ready && (dut.busy_counter != 7'd0))
